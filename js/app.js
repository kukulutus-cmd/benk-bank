...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  // Tampilkan Tombol Pengaturan jika Head Area
    ...
    <div id="login-modal" class="fixed inset-0 bg-slate-900 bg-opacity-80 flex items-center justify-center z-50 p-4">
      <div class="bg-white rounded-xl shadow-2xl max-w-md w-full p-6 border-t-4 border-red-700">
        ...
        <form id="form-login" class="space-y-4">
          ...
          <button type="submit" class="w-full bg-red-700 hover:bg-red-800 text-white font-bold py-2.5 rounded-lg text-xs transition-colors shadow-md">
            Masuk ke Portal
          </button>
        </form>
      </div>
...
function openSettingsModal() {
  ...
  document.getElementById("form-add-user").onsubmit = async (e) => {
    ...
    try {
      ...
      alert(`User Unit ${payload.email} berhasil didaftarkan! User sekarang bisa login.`);
      modal.classList.add("hidden");
      showLoading(false);
    } catch (err) {
      showLoading(false);
      alert("Gagal mendaftarkan user: " + err.message);
    }
  };
}
